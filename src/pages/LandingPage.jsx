  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, fontFamily: sans, overflowX: 'hidden' }}>
            ...
            {({ openConnectModal, mounted }) => {
              ...
              return (ready && (
                <button
                  ...
                  style={{ background: T.pink, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 13, fontWeight: 600, fontFamily: sans, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 7, transition: 'all .15s', flexShrink: 0 }}
                  onMouseEnter={e => { e.currentTarget.style.background = T.pinkL; }}
          ...
          <ConnectButton.Custom>
            {({ openConnectModal, mounted }) => {
              ...
              return (ready && (
                <button
                  onClick={openConnectModal}
                  style={{ background: T.pink, color: '#fff', border: 'none', borderRadius: 10, padding: '14px 32px', fontSize: 15, fontWeight: 600, fontFamily: sans, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10, boxShadow: `0 0 40px ${T.pinkGlow}`, transition: 'all .2s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = T.pinkL; e.currentTarget.style.transform = 'translateY(-1px)'; }}
          ...
          <ConnectButton.Custom>
            {({ openConnectModal, mounted }) => {
              ...
              return (ready && (
                <button
                  onClick={openConnectModal}
                  style={{ background: T.pink, color: '#fff', border: 'none', borderRadius: 10, padding: '14px 36px', fontSize: 15, fontWeight: 600, fontFamily: sans, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 10, position: 'relative', transition: 'all .2s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = T.pinkL; }}
                  onMouseLeave={e => e.currentTarget.style.background = T.pink}
                >
                  Get started
      ...
      <footer style={{ borderTop: `1px solid ${T.border}`, padding: '20px clamp(16px, 5vw, 48px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
        ...
        <span style={{ color: T.dim, fontSize: 12 }}>Built on Polkadot EVM · Solidity smart contracts · ERC20 USDC</span>
      </footer>
    </div>
  );
}
