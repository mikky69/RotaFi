...
export default function CreateModal({ onClose, onCreate }) {
  const [form, setForm] = useState({ name: '', memberCap: '8', depositAmount: '100', frequency: 'Monthly', usdcAddress: ADDRESSES.PolUSDC || '' });
  ...
  const submit = async () => {
    ...
    setErrorInfo('');
    try {
      // 1. Convert amount to USDC decimals (6 for native USDC, or our PolUSDC)
      const depositAmountScaled = BigInt(Math.floor(amt * 1_000_000));
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9000, padding: 24 }}
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, width: '100%', maxWidth: 460, animation: 'fadeUp .2s ease', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '24px 26px' }}>
          ...
          <input disabled={isBusy} style={{ ...inputStyle, marginBottom: 18 }} type="number" min="1" step="0.01" placeholder="100" value={form.depositAmount} onChange={set('depositAmount')} />

          <label style={labelStyle}>USDC contract address (ERC20)</label>
          <input disabled={isBusy} style={{ ...inputStyle, marginBottom: 20, fontFamily: "'DM Mono',monospace", fontSize: 13 }} placeholder="0x..." value={form.usdcAddress} onChange={set('usdcAddress')} />
